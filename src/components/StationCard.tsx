...
export default function StationCard({ station, isPlaying, onPlay }: StationCardProps) {
  ...
  useEffect(() => {
    ...
    if (img.complete && img.naturalWidth > 0 && !imgLoaded) {
      console.log(`[StationCard] Image already cached for ${station.name}, setting loaded state`);
      setImgLoaded(true);
    }
  ...
  useEffect(() => {
    if (logoSrc) {
      console.log(`[StationCard] ${station.name} - logoSrc set to:`, logoSrc);
    }
            ...
            {logoSrc && !imgError ? (
              <>
                <img
                  ...
                  onLoad={() => {
                    console.log(`[StationCard] Image loaded successfully for ${station.name}`);
                    setImgLoaded(true);
                  }}
                  onLoadStart={() => {
                    console.log(`[StationCard] Image load started for ${station.name}:`, logoSrc);
                  }}
                  loading="eager"
