...
export default function Player({ 
  ...
  useEffect(() => {
    ...
    const callback = (isSpeaking: boolean) => {
      ...
      if (isSpeaking) {
        ...
        console.log('[Player] TTS started, storing original volume:', volume, 'will duck to:', volume * 0.3);
      } else {
        console.log('[Player] TTS ended, will restore volume to:', originalVolumeRef.current);
      }
    };
    setTTSStateChangeCallback(callback);
    console.log('[Player] TTS callback registered');
  }, [volume, isTTSSpeaking]);
  ...
  */
  ...
  const { isConnected: wakeWordConnected, error: wakeWordError, getDetector } = useWakeWordDetector({
    ...
    onDetection: (detection) => {
      ...
      if (timeSinceLastDetection < DEBOUNCE_MS) {
        console.log(`[WakeWord] Detection ignored (debounced, ${timeSinceLastDetection}ms since last)`);
        return;
      }
      
      lastDetectionTimeRef.current = now;
      console.log('[WakeWord] Detection triggered:', detection);
      
      // Wake word detected - trigger command recording
      const voiceControl = getVoiceControl();
      if (voiceControl.isSupported()) {
        console.log('[WakeWord] Starting command recording');
        // Get the shared stream from wake word detector if available
        const wakeWordDetector = getDetector();
        const sharedStream = wakeWordDetector?.getMediaStream() || null;
        // Start recording for Gemini AI processing, using shared stream if available
        voiceControl.startCommandRecording(sharedStream || undefined);
      }
    },
  ...
  useEffect(() => {
    if (wakeWordConnected) {
      console.log('[WakeWord] Connected to WebSocket server');
    } else if (wakeWordError) {
      console.error('[WakeWord] Error:', wakeWordError);
    }
  }, [wakeWordConnected, wakeWordError]);
          ...
          switch (command.type) {
            case 'play':
              if (command.stationName && onPlayStation) {
                console.log(`[Player] Executing play command for station: "${command.stationName}"`);
                onPlayStation(command.stationName);
                // TTS is already handled by voiceControl.ts, no need to call it again
              } else {
                console.warn('[Player] Play command received but stationName is missing or onPlayStation is not available');
              }
              break;
            case 'next':
              if (onNextStation) {
